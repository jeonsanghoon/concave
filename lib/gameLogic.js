...
export function addSpectator(room, playerId, spectatorName) {
  if (room.blackPlayer === playerId) {
  ...
  if (!existing) {
    room.spectators.push({ id: playerId, name: spectatorName || '관전자' });
  }
  room.updatedAt = Date.now();
...
export function roomToClient(room, playerId) {
  ...
  const isSpectator = !myColor && (room.status === 'playing' || room.status === 'finished');
  return {
    id: room.id,
    ...
    myColor,
    role: myColor === BLACK ? 'black' : myColor === WHITE ? 'white' : isSpectator ? 'spectator' : null,
    isSpectator,
